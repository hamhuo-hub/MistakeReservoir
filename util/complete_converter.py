...
def check_start_trigger(text):
    """检查是否触发删除模式 (返回 match 对象)"""
    return ANSWER_REGEX.search(text)
...
def clean_docx_complete(input_path, output_path):
    ...
    for block in blocks:
        ...
        for p in paragraphs:
            ...
            # 3. 检查：开始删除触发器 (答案/解析)
            # -------------------------------------------------------
            match_obj = check_start_trigger(text)
            if match_obj:
                is_deleting = True
                ...
                if start_idx == 0:
                    ...
                    # 保留触发词之前的内容 (如: "题目xxx (A) ... 【答案】")
                    p.text = text[:start_idx].strip()
                continue
